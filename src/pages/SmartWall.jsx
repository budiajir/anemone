...
import { Box, Smartphone, Zap, Check, FileText, SmartphoneIcon, Settings, BarChart2, ShoppingCart, CheckCircle2, Cpu, Route, Sparkles, Layers } from 'lucide-react';
...
export default function SmartWall() {
  ...
  // Smart Wall Kit Products matching store products
  const smartWallProducts = [
    {
      id: 16,
      name: "Essential LED Kit",
      slug: "essential-led-kit",
      category: "Smart Wall Kit",
      price: 3500000,
      description: "Entry-level smart wall kit dengan 50 modul LED addressable + Anemone Controller Hub v2.",
      shortDescription: "50 LED modules + controller for smart wall basics",
      images: ["/images/smartwall.jpg"],
      variants: [{ name: "LED Color", options: ["RGB Full Spectrum", "Warm White Only"] }],
      popular: false
    },
    {
      id: 17,
      name: "Pro Holds Bundle",
      slug: "pro-holds-bundle",
      category: "Smart Wall Kit",
      price: 5800000,
      description: "Paket paling direkomendasikan. 30 holds ergonomis dengan port LED khusus + Pro Controller Hub.",
      shortDescription: "30 LED-integrated holds + Pro controller",
      images: ["/images/smartwall.jpg"],
      variants: [{ name: "Hold Style", options: ["Mixed Set", "Crimp Focus", "Jug Focus"] }],
      popular: true
    },
    {
      id: 18,
      name: "Full Ecosystem Set",
      slug: "full-ecosystem-set",
      category: "Smart Wall Kit",
      price: 8900000,
      description: "Solusi terlengkap: 100 modul LED, 50 holds premium, sensor tekanan top-out & lisensi aplikasi seumur hidup.",
      shortDescription: "Complete ecosystem: 100 LEDs, 50 holds, sensors, lifetime app",
      images: ["/images/smartwall.jpg"],
      variants: [{ name: "Wall Size", options: ["Small (8m²)", "Medium (16m²)", "Large (24m²)"] }],
      popular: false
    }
  ];

  const handleAddToCart = (pkg) => {
    // Find matching product in catalog or fallback to package data
    const catalogProduct = products.find((p) => p.id === pkg.id) || pkg;
    const defaultVariants = (catalogProduct.variants || []).reduce(
      (acc, v) => ({ ...acc, [v.name]: v.options[0] }),
      {}
    );

    addItem(catalogProduct, defaultVariants, 1);
    setAddedProductId(pkg.id);
